...
patch = 1
name_log_dir = 'exp_test_residual' + str(patch)
name_model = 'model_test_residual' + str(patch)
...
#########################################   
ep_step = 20 # ogni quante epoche salvare i plot su tensorboard
...
class patchFNO_DarcyL(nn.Module):
    ...
    def forward(self, x):
        ...
        # ora x è un tensore di dimensione: (n_samples)*(n_patch)*(nx)*(ny)*(3)
        
        x = x.permute(0, 3, 1, 2) # (n_samples)*(3)*(nx)*(ny)
...
if __name__ == '__main__':
    ...
    a_train, u_train = a_train[idx_train, patch ,:, :], u_train[idx_train, patch, :, :]    
    ...
    a_test, u_test = a_test[idx_test, patch, :, :], u_test[idx_test, patch, :, :]
    # Gaussian pointwise normalization
