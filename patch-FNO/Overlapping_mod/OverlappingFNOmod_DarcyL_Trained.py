...
Path = "model_OverlappingFNOmod_DarcyL_Residual"

#########################################
# numero esempio da plottare
#########################################
idx = [42]
...
if multiple_mesh:
    ...
    s = 1 # parametro mesh: s=3 --> 70, s=1 --> 211
...
if __name__ == '__main__':
    ...
    im = plt.imshow(set_nan(esempio_prova.squeeze()))
    plt.colorbar(im)
    plt.show()
    ...
    #### Soluzione approssimata con FNO
    # Calcolo soluzione
    t1 = default_timer()
    ...
    out_test = out_test.unsqueeze(0)
    soluzione_test = soluzione_test.unsqueeze(0)
    #### errore in norma L2
    err_L2 = L2Loss()(out_test, soluzione_test).item()
    print('Errore in norma L2 =', err_L2)
    #### errore relativo in norma L2
    err_rel_L2 = L2relLoss()(out_test, soluzione_test).item()
    print('Errore relativo in norma L2 =', err_rel_L2)
    #### errore relativo in norma H1
    err_rel_H1 = H1relLoss()(out_test, soluzione_test).item()
    ...
    #### Plot
    # sol esatta
    im = plt.imshow(set_nan(soluzione_test.squeeze()))
    plt.colorbar(im)
    plt.show()
    # sol FNO
    im = plt.imshow(set_nan(out_test.squeeze()))
    plt.colorbar(im)
    plt.show()
    #differenza
    im = plt.imshow(set_nan(diff.squeeze()))
    plt.colorbar(im)
    plt.show()
