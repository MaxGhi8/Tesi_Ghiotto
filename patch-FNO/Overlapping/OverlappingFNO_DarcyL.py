    ...
    def forward(self, x_1, x_2):
        ...
        for i in range(self.L):
            if arc == 'Tran':
                ...
                xx_1, xx_2 = sum_patch(xx_1, xx_2)
                # Prima componente
                xx_1 = activation(self.ws1_1[i](xx_1))
                if self.BN: 
                    xx_1 = self.BSs_1[i](xx_1)
                xx_1 = self.ws2_1[i](xx_1)
                xx_1 = activation(xx_1) # 
                x_1 = x_1 + xx_1
                # Seconda componente
                xx_2 = activation(self.ws1_2[i](xx_2))
                if self.BN: 
                ...
                x_1 = self.fouriers_1[i](x_1)
                x_2 = self.fouriers_2[i](x_2)
    ...
    for ep in range(epochs):
        ...
        if ep == 0:
            ...
            for i in range(n_idx):
                ...
                ax[i].set(xlabel = 'x')
                # figura
                im = ax[i].imshow(esempio_1_test[i])
                fig.colorbar(im, ax = ax[i])
            if plotting:
                plt.show()
            writer.add_figure('coeff a_1(x)', fig, 0)
            # Seconda componente
            fig, ax = plt.subplots(1, n_idx, figsize = (18, 4))
            fig.suptitle('coeff a_2(x)')
            ax[0].set(ylabel = 'y')
            for i in range(n_idx):
                # Turn off tick labels
                ax[i].set_yticklabels([])
                ...
                ax[i].set(xlabel = 'x')
                # figura
                im = ax[i].imshow(esempio_2_test[i])
                fig.colorbar(im, ax = ax[i])
            if plotting:
                plt.show()
            writer.add_figure('coeff a_2(x)', fig, 0)
