...
class L2relLoss():
    ...
    def rel(self, x, y):
        ...
        x = torch.cat((x[:, :, :int(n_y/2)], x[:, :int(n_x/2), int(n_y/2):]), dim = 1)
        y = torch.cat((y[:, :, :int(n_y/2)], y[:, :int(n_x/2), int(n_y/2):]), dim = 1)
...
if __name__ == '__main__':
    ...
    if multiple_mesh:
        ...
        a_train = torch.cat( (torch.cat((a_train[:, 1, :, :], a_train[:, 2, :, :]), dim = 1), 
                              torch.cat((a_train[:, 0, :, :], torch.zeros_like(a_train[:, 0, :, :])), dim = 1)), dim = 2)
        u_train = torch.cat( (torch.cat((u_train[:, 1, :, :], u_train[:, 2, :, :]), dim = 1), 
                              torch.cat((u_train[:, 0, :, :], torch.zeros_like(u_train[:, 0, :, :])), dim = 1)), dim = 2)
        a_train = a_train.unsqueeze(-1)
        # extraction test data
        a_test, u_test = a_test[:, :, ::s, ::s], u_test[:, :, ::s, ::s]
        _, _, nx_test, ny_test = a_test.size()
        a_test = torch.cat( (torch.cat((a_test[:, 1, :, :], a_test[:, 2, :, :]), dim = 1), 
                              torch.cat((a_test[:, 0, :, :], torch.zeros_like(a_test[:, 0, :, :])), dim = 1)), dim = 2)
        u_test = torch.cat( (torch.cat((u_test[:, 1, :, :], u_test[:, 2, :, :]), dim = 1), 
                              torch.cat((u_test[:, 0, :, :], torch.zeros_like(u_test[:, 0, :, :])), dim = 1)), dim = 2)
    ...
    for ep in range(epochs):
        ...
        with torch.no_grad(): # per efficienza
            for a, u in test_loader:
                ...
                out = model.forward(a)   
                test_l2 += L2relLoss()(out, u.squeeze()).item()
                test_h1 += H1relLoss()(out, u.squeeze()).item()
        ...
        if ep == 0:
            ...
            if multiple_mesh:
                ...
                esempio_test = a_test[idx].squeeze()
            
            fig, ax = plt.subplots(1, n_idx, figsize = (18, 6))
            ...
            if multiple_mesh:
                ...
                soluzione_test = u_test[idx].squeeze()
        ...
        if ep % ep_step == 0:
            with torch.no_grad(): # no grad per efficienza
                out_test = model(esempio_test.unsqueeze(-1))
