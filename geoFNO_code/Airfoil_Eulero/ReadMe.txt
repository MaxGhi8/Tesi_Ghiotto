...
- Airfoil_geoFNO.py --> file di codice python utilizzato per creare una rete FNO per risolvere il problema di 
			Eulero per la airfoil
...
- folder data --> Come dati di input si prendono le coordinate della mesh che discretizza la airfoil che si trovano rispettivamente
		  in data/naca/NACA_Cylinder_X.npy e data/naca/NACA_Cylinder_Y.npy. La struttura della mesh che discretizza l'airfoil
		  è strutturata, in particolare nel nostro esempio ci sono 211 punti perimetrali e 51 strati. L'output invece è il 
		  valore del numero di Mach sui vari punti della mesh e i dati approssimati sono nel file data/naca/NACA_Cylinder_Q.npy.
...
- Plasticity_geoFNO_Trained.py --> codice per fare prove con le reti già allenate, utilizzando i parametri contenuti nei model_...

- Didascalia ... =
	- exp_plasticity --> FNO con 1000 training instances, 200 test instances, risoluzione originale (211*51)
		 ...
		 {train_loss = 0.02024, test_loss_L2 = 0.02113, test_loss_H1 = 0.04498}
