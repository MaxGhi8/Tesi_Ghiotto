...
class UnitGaussianNormalizer(object):
    """ 
    Initial normalization su x che � un tensore di 
...
class Airfoil_geoFNO(nn.Module):
    def __init__(self, d_a, d_v, d_u, L, modes1, modes2):
        ...
        input is the coordinate of the mesh and output si the Mach number
        on every point of the mesh
        input shape: (batchsize, x=211, y=51, c=2)
        poi all'input aggiungo i valori di x e y --> d_a = 4
        output shape: (batchsize, x=211, y=51, c=1)
        """
        self.modes1 = modes1
        self.modes2 = modes2
        self.d_a = d_a
...
if __name__ == '__main__':
    ...
    for ep in range(epochs):
        ...
        # Estraggo solo i pi� vicini alla airfoil
        X_small = X[:, nx:-nx, :ny]
        Y_small = Y[:, nx:-nx, :ny]
        ...
        if ep == 0:
            ...
            fig, ax = plt.subplots(2, n_idx, figsize = (18, 4))
            ...
            for i in range(n_idx):
                ...
                ax[0, i].scatter(X[i], Y[i], size)
                ax[1, i].scatter(X_small[i], Y_small[i], size)            
            if plotting:
                plt.show()
            ...
            for i in range(n_idx):
                ...
                im1 = ax[0, i].pcolormesh(X[i], Y[i], sol_test_i, shading='gouraud')
                fig.colorbar(im1, ax = ax[0, i])
                im2 = ax[1, i].pcolormesh(X_small[i], Y_small[i], sol_test_i[nx:-nx, :ny], shading='gouraud') 
        ...
        if ep % ep_step == 0:
            ...
            for i in range(n_idx):
                ...
                im1 = ax[0, i].pcolormesh(X[i], Y[i], out_test_i, shading='gouraud')
                fig.colorbar(im1, ax = ax[0, i])
                im2 = ax[1, i].pcolormesh(X_small[i], Y_small[i], out_test_i[nx:-nx, :ny], shading='gouraud')     
                fig.colorbar(im2, ax = ax[1, i])
            if plotting:
                plt.show()
            ...
            for i in range(n_idx):
                ...
                # figura
                diff_i = diff[i]
                im1 = ax[0, i].pcolormesh(X[i], Y[i], diff_i, shading='gouraud')
                fig.colorbar(im1, ax = ax[0, i])
                im2 = ax[1, i].pcolormesh(X_small[i], Y_small[i], diff_i[nx:-nx, :ny], shading='gouraud') 
                fig.colorbar(im2, ax = ax[1, i])
            if plotting:
                plt.show()
