"""
geoFNO for hyper-elastic material with O-mesh
"""
...
outout_sigma = '../data/Omesh/Random_UnitCell_Deform_sigma_10_interp.npy'
...
s1 = 64 # numero di punti discretizzazione asse x
...
class Elasticity_Omesh_geoFNO(nn.Module):
    def __init__(self, d_a, d_v, d_u, L, modes1, modes2):
        ...
        input is the coordinate of the mesh and output is the stress
        input shape: (batchsize, x=129, y=129, c=2)
        d_a = 4 poiché aggiungo le coordinate di x e y
    ...
    for ep in range(epochs):
        ...
        if ep == 0:
            ...
            for i in range(n_idx):
                ...
                im = ax[0, i].scatter(XY_og[i, :, 0].squeeze(), XY_og[i, :, 1].squeeze(), size, sol_test_og_i, edgecolor='w', lw=0.1, **lims)
                fig.colorbar(im, ax = ax[0, i])
                im = ax[1, i].scatter(X[i], Y[i], size, sol_test_i, edgecolor='w', lw=0.1, **lims)
