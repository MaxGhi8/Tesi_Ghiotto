"""
geoFNO for hyper-elastic material with D-mesh
"""
...
#### Estrazione dei dati
Input_x = '../data/Rmesh/Random_UnitCell_Deform_Grid_X_10_interp.npy'
Input_y = '../data/Rmesh/Random_UnitCell_Deform_Grid_Y_10_interp.npy'
outout_sigma = '../data/Rmesh/Random_UnitCell_Deform_Grid_sigma_10_interp.npy'
...
s1 = 41 # numero di punti discretizzazione asse x
s2 = 41 # numero di punti discretizzazione asse y
...
if __name__ == '__main__':
    ...
    # Importo la griglia originale e la soluzione sulla griglia originale
    # non con la griglia Omesh
    sigma_og = np.load("../data/Meshes/Random_UnitCell_sigma_10.npy").transpose()[ntrain:ntrain+ntest]
    XY_og = np.load("../data/Meshes/Random_UnitCell_XY_10.npy").transpose(2,0,1)[ntrain:ntrain+ntest]
    ...
    for ep in range(epochs):
        ...
        if ep == 0:
            ...
            #### Mesh originale
            XY_og = XY_og[idx]
            sol_test_og = sigma_og[idx]
            #### Valori della mesh
            X = a_test[idx, : ,:, 0].numpy()
            Y = a_test[idx, :, :, 1].numpy()

            fig, ax = plt.subplots(2, n_idx, figsize = (18, 8))
            fig.suptitle('Mesh iniziale originale e O-mesh')
            ax[0, 0].set(ylabel = 'y')
            ax[1, 0].set(ylabel = 'y')
            for i in range(n_idx):
                ...
                ax[0, i].scatter(XY_og[i, :, 0].squeeze(), XY_og[i, :, 1].squeeze(), size)
                ax[1, i].scatter(X[i], Y[i], size)            
            if plotting:
                plt.show()
            writer.add_figure('Mesh iniziale originale e O-mesh', fig, 0)
            
            #### Soluzione esatta
            sol_test = u_test[idx]
            fig, ax = plt.subplots(2, n_idx, figsize = (18, 8))
            fig.suptitle('Soluzione esatta griglia originale e O-mesh')
            ax[0, 0].set(ylabel = 'y')
            ax[1, 0].set(ylabel = 'y')
            for i in range(n_idx):
                ax[0, i].set(xlabel = 'x')
                ...
                sol_test_og_i = sol_test_og[i]
                ax[0, i].scatter(XY_og[i, :, 0].squeeze(), XY_og[i, :, 1].squeeze(), size, sol_test_og_i, edgecolor='w', lw=0.1, **lims)
                ax[1, i].scatter(X[i], Y[i], size, sol_test_i, edgecolor='w', lw=0.1, **lims)
            if plotting:
                plt.show()
            writer.add_figure('Soluzione esatta griglia originale e O-mesh', fig, 0)
        ...
        if ep % ep_step == 0:
            ...
            fig.suptitle('Soluzione approssimata O-mesh')
            ax[0].set(ylabel = 'y')
            for i in range(n_idx):
                ax[i].set(xlabel = 'x')
                # Turn off tick labels
                ...
                ax[i].scatter(X[i], Y[i], size, out_test_i, edgecolor='w', lw=0.1, **lims)
            if plotting:
                plt.show()
            writer.add_figure('Soluzione approssimata O-mesh', fig, ep)
            
            #### Differenza
            diff = np.abs(sol_test - out_test)
            fig, ax = plt.subplots(1, n_idx, figsize = (18, 4))
            fig.suptitle('Differenza O-mesh')
            ax[0].set(ylabel = 'y')
            for i in range(n_idx):
                ax[i].set(xlabel = 'x')
                # Turn off tick labels
                ...
                # figura
                diff_i = diff[i]
                ax[i].scatter(X[i], Y[i], size, np.abs(diff_i), edgecolor='w', lw=0.1, **lims)
            if plotting:
                plt.show()
            writer.add_figure('Differenza O-mesh', fig, ep)
