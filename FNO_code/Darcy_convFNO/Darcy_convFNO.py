"""
Implementazione di FNO convolutional per la risoluzione del problema di Darcy 2D.
"""
...
name_log_dir = 'exp_Darcy_test'
name_model = 'model_Darcy_test'
...
ep_step = 25 # ogni quante epoche salvare i plot su tensorboard
...
class FourierLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.fft.irfft2(out_ft, s=(M, N))
...
#########################################
# Convolutional lifting
#########################################
class Convol(nn.Module):
    """ Due conv layer """
    def __init__(self, in_channels, mid_channels, out_channels, ker_size, padding):
        super(Convol, self).__init__()
        self.conv1 = nn.Conv2d(in_channels, mid_channels, kernel_size = ker_size, padding = padding)
        self.conv2 = nn.Conv2d(mid_channels, out_channels, kernel_size = ker_size, padding = padding)

    def forward(self, x):
        x = self.conv1(x) # affine transformation
        x = activation(x) # activation function
        x = self.conv2(x) # affine transformation
        return x
    
#########################################
# FNO with some tricks for Darcy
#########################################
class Darcy_convFNO(nn.Module):
    """ 
    ...
    def __init__(self, d_a, d_v, d_u, L, modes1, modes2):
        ...
        """
        super(Darcy_convFNO, self).__init__()
        ...
        # Encoder
        self.p = Convol(self.d_a, self.d_v // 2, self.d_v, 3, 1) # input features is d_a=3: (a(x, y), x, y)
    ...
    def forward(self, x):
        ...
        # ora x è un tensore di dimensione: (n_samples)*(n_x)*(n_y)*(d_a)
        
        x = x.permute(0, 3, 2, 1) # (n_samples)*(d_a)*(n_x)*(n_y)
...
if __name__ == '__main__':
    ...
    model = Darcy_convFNO(d_a, d_v, d_u, L, modes, modes)
    # model.to(mydevice)
    ...
    for ep in range(epochs):
        ...
        writer.add_scalars('Darcy_convFNO', {'Train_loss': train_loss,
                                               'Test_loss_l2': test_l2,
